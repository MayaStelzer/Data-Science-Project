...
class MyRandomForestClassifier:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.N):
            ...
            X_sample = [copy.deepcopy(X_train[i]) for i in bootstrap_indices]
            y_sample = [copy.deepcopy(y_train[i]) for i in bootstrap_indices]
            ...
            oob_indices = [i for i in range(len(X_train)) if i not in bootstrap_set]

            if oob_indices:
                X_val = [[X_train[i][j] for j in features] for i in oob_indices]
                y_val = [y_train[i] for i in oob_indices]
            else:
                # use entire training set if no OOB samples
                X_val = X_reduced
                y_val = y_sample
